    ...
    return (
        <Dialog>
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <DialogFooter>
                  <DialogTrigger asChild>
                    <Button onClick={handleCalculate} aria-label={`Calculate ${currentOp.title}`}>Calculate</Button>
                   </DialogTrigger>
                </DialogFooter>
            </DialogContent>
        </Dialog>
...
const NewMatrixButton = ({ onClick }: { onClick: () => void }) => (
  <Button variant="outline" onClick={onClick} className="w-full min-h-[160px] border-dashed" aria-label="Add new matrix">
    ...
    <Card className="relative flex flex-col">
      ...
      <CardContent className="p-2 pt-0 flex-1">
       ...
       <CardFooter className="p-2 flex-wrap gap-1">
         <Button size="xs" variant="outline" onClick={() => fillMatrix(id, 0)} aria-label={`Clear matrix ${name}`}>Clear</Button>
         <Button size="xs" variant="outline" onClick={() => fillMatrix(id, 1)} aria-label={`Fill matrix ${name} with ones`}>1s</Button>
         <Button size="xs" variant="outline" onClick={() => fillMatrix(id, 'random')} aria-label={`Fill matrix ${name} with random numbers`}>Rand</Button>
         <Button size="xs" variant="outline" onClick={() => fillMatrix(id, 'identity')} aria-label={`Fill matrix ${name} as identity matrix`}>Identity</Button>
       </CardFooter>
    </Card>
  );
};
